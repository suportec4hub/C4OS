...
/* how many months to show in trend charts based on range */
function getMonths(rangeDays) {
  const n = rangeDays <= 30 ? 2 : rangeDays <= 60 ? 3 : 6;
  const now = new Date();
...
function getFromDate(range, customStart) {
  if (range === "custom" && customStart) return new Date(customStart);
  const d = new Date();
  d.setDate(d.getDate() - Number(range));
  return d;
}
function getToDate(range, customEnd) {
...
/* ── Range Selector ──────────────────────────────────────────── */
function RangeSelector({ range, setRange, customStart, setCustomStart, customEnd, setCustomEnd }) {
  return (
      ...
      {range === "custom" && (
        <div style={{ display:"flex", alignItems:"center", gap:4 }}>
          <input type="date" value={customStart} onChange={e => setCustomStart(e.target.value)}
            style={{ border:`1.5px solid ${L.line}`, borderRadius:8, padding:"3px 8px", fontSize:11, color:L.t2, background:L.white, outline:"none" }}/>
...
export default function PageDashboard({ user }) {
  ...
  useEffect(() => {
    ...
    fetchAll(user.empresa_id);
  }, [user?.empresa_id]);

  async function fetchAll(empresa_id) {
    setLoading(true);
    const [
      { data: empresa },
      { data: leads },
  ...
  const dados = useMemo(() => {
    ...
    /* date bounds */
    const effectiveDays = range === "custom"
      ? (customStart && customEnd ? Math.round((new Date(customEnd) - new Date(customStart)) / 86400000) : 30)
      : Number(range);
    const from = getFromDate(range, customStart);
    const to   = getToDate(range, customEnd);
    const months = getMonths(effectiveDays);
    ...
    const dealsIn = dealsAll.filter(d => {
      const dt = new Date(d.updated_at || d.created_at);
      return dt >= from && dt <= to;
    });

    /* ── KPIs ── */
    const novosLeads     = leadsIn.length;
    const totalLeads     = leadsAll.length;
    const totalFechados  = dealsAll.filter(d => ["fechado","ganho"].includes((d.etapa||"").toLowerCase())).length;

    const dealsFechadosIn = dealsIn.filter(d => ["fechado","ganho"].includes((d.etapa||"").toLowerCase()));
    const faturamento = dealsFechadosIn.reduce((s, d) => s + (parseFloat(d.valor) || 0), 0);

    const dealsFechadosTodos = dealsAll.filter(d => ["fechado","ganho"].includes((d.etapa||"").toLowerCase()));
    const ticketMedio = dealsFechadosTodos.length
      ? dealsFechadosTodos.reduce((s, d) => s + (parseFloat(d.valor) || 0), 0) / dealsFechadosTodos.length
      : 0;
    const pipeline = dealsAll
      .filter(d => !["perdido","fechado","ganho"].includes((d.etapa||"").toLowerCase()))
      .reduce((s, d) => s + (parseFloat(d.valor) || 0), 0);

    /* ── Receita por mês (trend chart — all time, last N months) ── */
    const receitaMap = Object.fromEntries(months.map(m => [m.key, 0]));
    dealsFechadosTodos.forEach(d => {
      const key = (d.updated_at || d.created_at)?.slice(0, 7);
      if (receitaMap[key] !== undefined) receitaMap[key] += (parseFloat(d.valor) || 0);
    });
    const receitaData = months.map(m => ({ m: m.label, r: receitaMap[m.key] || 0, meta: mrr || 0 }));

    /* ── Leads por mês (trend chart) ── */
    const leadsMap = Object.fromEntries(months.map(m => [m.key, 0]));
    leadsAll.forEach(l => {
    ...
    /* ── Canais de origem (filtered by period) ── */
    const origemCount = {};
    leadsIn.forEach(l => {
      const o = (l.origem || "Outro").trim();
      origemCount[o] = (origemCount[o] || 0) + 1;
    });
    const canaisRaw   = Object.entries(origemCount).map(([name, value]) => ({ name, value, c: CORES_ORIGEM[(name || "").toLowerCase()] || L.t4 })).sort((a, b) => b.value - a.value);
    const canaisTotal = canaisRaw.reduce((s, c) => s + c.value, 0);
    const canaisComPct = canaisRaw.map(c => ({ ...c, pct: pct(c.value, canaisTotal) }));

    /* ── Funil (filtered by period) ── */
    const statusCount = {};
    leadsIn.forEach(l => {
  ...
  /* ── period label for KPI subtitles ── */
  ...
  const kpis = dados ? [
    {
      l: "MRR",         v: dados.mrr ? fmt(dados.mrr) : "—",
      d: dados.mrr ? "Receita recorrente" : "Configure em Empresa",
      up: dados.mrr > 0, s: "mês atual", c: L.teal, bg: L.tealBg,
    },
    {
      l: "Novos Leads", v: String(dados.novosLeads),
      d: `${dados.totalLeads} total`,
      up: dados.novosLeads > 0, s: periodLabel, c: L.copper, bg: L.copperBg,
    },
    {
      l: "Faturamento", v: dados.faturamento ? fmt(dados.faturamento) : "—",
      d: `${dados.totalFechados} deals fechados`,
      up: dados.faturamento > 0, s: periodLabel, c: L.green, bg: L.greenBg,
    },
    {
      l: "Ticket Médio",v: dados.ticketMedio ? fmt(dados.ticketMedio) : "—",
      d: "média deals fechados",
      up: dados.ticketMedio > 0, s: "histórico", c: L.copper, bg: L.copperBg,
    },
    {
      l: "Pipeline",    v: dados.pipeline ? fmt(dados.pipeline) : "—",
      d: "oportunidades abertas",
      up: dados.pipeline > 0, s: "total em aberto", c: L.teal, bg: L.tealBg,
    },
    {
  ...
  return (
    <Fade>
      ...
      {dados.totalLeads === 0 && dados.mrr === 0 && (
        <div style={{ background:`linear-gradient(135deg,${L.tealBg},${L.copperBg})`, border:`1px solid ${L.teal}22`, borderRadius:12, padding:"14px 18px", marginBottom:16, display:"flex", alignItems:"center", gap:12 }}>
          ...
          <div>
            ...
            <div style={{ fontSize:11, color:L.t3, marginTop:2 }}>Cadastre leads, crie deals no pipeline e configure o MRR em <b>Minha Empresa</b> para ver os indicadores reais.</div>
          </div>
        </div>
      )}
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:8, marginBottom:16 }}>
        <div style={{ fontSize:11, color:L.t4, fontWeight:600, textTransform:"uppercase", letterSpacing:"1px" }}>Período de análise</div>
        <RangeSelector
      ...
      <Grid cols="2fr 1fr" gap={12} mb={12} responsive>
        <Card title="Receita vs Meta" sub={`últimos meses`}>
          {hasReceitaData ? (
        ...
        <Card title="Canais de Origem" sub={periodLabel}>
          {hasCanaisData ? (
            <>
      ...
      <Grid cols={3} gap={12} responsive>
        <Card title="Funil de Conversão" sub={periodLabel}>
          {hasFunilData ? (
