...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ── CHATS_UPDATE ────────────────────────────────────────────────────────
    // Não zera badge. O evento chega apenas com {remoteJid, instanceId}, sem
    // unreadCount, e a verificação via findChats que preenchia essa lacuna
    // apagava notificação de mensagem não lida: a Evolution é um dispositivo
    // conectado e consome a mensagem ao recebê-la, então o unreadCount dela
    // fica 0 sem ninguém ter aberto a conversa. Medido em produção, as
    // conversas zeradas por esse caminho não tinham recibo de leitura nenhum.
    //
    // Leitura de verdade chega como recibo em messages.update/message.ack
    // (status READ ou PLAYED), tratado logo abaixo, ou pela abertura da
    // conversa no C4OS.
